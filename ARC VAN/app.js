...
const ntfyQrSrc = `https://api.qrserver.com/v1/create-qr-code/?size=312x312&margin=8&data=${encodeURIComponent(ntfyUrl)}`;
ntfyDriverQr.src = ntfyQrSrc;
ntfyStudentQr.src = ntfyQrSrc;
qrUrl.textContent = ntfyUrl;
...
document.querySelectorAll('.student-stop-btn').forEach((button) => button.addEventListener('click', () => {
  ...
  if (button.dataset.pickup === 'Other') {
    otherPickup.classList.add('visible');
    otherPickupInput.focus();
    return;
  }
  otherPickup.classList.remove('visible');
  ridePickupInput.value = button.dataset.pickup;
}));


document.querySelector('#set-other-pickup').addEventListener('click', () => {
  const pickup = otherPickupInput.value.trim();
  if (!pickup) {
    otherPickupInput.focus();
    return;
  }
  ridePickupInput.value = pickup;
  otherPickup.classList.remove('visible');
});
...
rideRequestForm.addEventListener('submit', (event) => {
  ...
  const pickup = ridePickupInput.value;
...
async function sendDriverAlert() {
  ...
  const pin = sessionStorage.getItem("driverPin") || "045048";

  statusMsg.innerText = "Sending push alert...";
  statusMsg.style.color = "#555";

  try {
    const res = await fetch("/api/driver/broadcast", {
      method: "POST",
      headers: {
    ...
    if (res.ok) {
      statusMsg.innerText = " Alert successfully published!";
      statusMsg.style.color = "green";
    } else {
      statusMsg.innerText = `Failed: ${data.detail || "Error sending alert"}`;
      statusMsg.style.color = "red";
    }
  } catch (err) {
    statusMsg.innerText = "Network error connecting to server.";
    statusMsg.style.color = "red";
  }
}
async function sendVanAlert() {
  const stopSelect = document.getElementById("broadcast-stop") || document.getElementById("stop-select");
  const statusSelect = document.getElementById("broadcast-status") || document.getElementById("status-select");
  
  const stop = stopSelect ? stopSelect.value : "Main Gate";
  const message = statusSelect ? statusSelect.value : "Van arriving now";

  console.log("Submitting alert to backend:", { stop, message });
  ...
  try {
    const response = await fetch("/api/driver/broadcast", {
      ...
      headers: {
        "Content-Type": "application/json",
        "X-Driver-PIN": "045048"   // Must match DRIVER_PIN in main.py
      },
      body: JSON.stringify({
        current_stop: stop,
    ...
    if (response.ok) {
      alert("Alert successfully sent to Ntfy!");
    } else {
      alert("Error: " + (result.detail || "Broadcast failed"));
    }
  } catch (error) {
